import type { Fields } from '../lib/field';
import type { FormConfig, FormState, FormValues } from '../lib/types';
import { ValidationMode } from '../lib/types';
...
export const useFormState = <P extends Plugins, F extends Fields>(
  ...
  useAsyncReducer(initialState, {
    ...
    change: (
      ...
      const newValue = field.plugin.parse
        ? field.plugin.parse(targetValue)
        : targetValue;
          ...
          ...(fieldIndex === undefined
            ? {
                ...
                error: shouldValidate
                  ? validateField(field, state, newValue)
                  : error,
              }
            : {
      ...
      if (config.changeEffect) {
        ...
        if (Util.isPromise(changeEffectResult))
          Promise.resolve(changeEffectResult);
      }
    ...
    blur: (formState, fieldName: keyof F, fieldIndex?: number) => {
      ...
      const { field, value, touched, error, state } = stateField;
      const newValue =
        fieldIndex === undefined ? value : (value as unknown[])[fieldIndex];
      ...
      const v =
        ...
        ValidationMode.AFTER_BLUR;
      const shouldValidate =
        v === ValidationMode.ON_BLUR || v === ValidationMode.AFTER_BLUR;
    ...
    submit: async (formState) => {
      ...
      if (canSubmit && config.onSubmit) {
        ...
        if (Util.isPromise(onSubmitResult))
      ...
      if (config.submitEffect) {
        ...
        if (Util.isPromise(submitEffectResult))
          Promise.resolve(submitEffectResult);
      }

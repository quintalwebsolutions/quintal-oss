...
export type Column<
  ...
  TColumnName extends keyof TRow | undefined = undefined,
> = TColumnName extends keyof TRow
  ? ColumnBase<TRow, TRow[TColumnName]> &
      // TODO fix this type (it works in practice)
      // @ts-expect-error We should somehow annotate that ColumnInput returns a Record<keyof TRow, object>
      ColumnMapInputFromPlugins<TPlugins>[TColumnName]
  : ColumnBase<Row, Row[keyof Row]> & ColumnInputFromPlugins<TPlugins>;

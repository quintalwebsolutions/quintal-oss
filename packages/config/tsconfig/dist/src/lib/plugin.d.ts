...
export type Plugin<TInternalValue, TPluginArgs extends PluginArgs = Record<string, unknown>> = {
    ...
    serializedValue: keyof TPluginArgs extends 'serializedValue' ? TPluginArgs['serializedValue'] : TInternalValue;
...
export type PluginObject<TPlugin extends AnyPlugin> = (null extends TPlugin['internalValue'] | TPlugin['emptyValue'] ? {
    ...
    parse?: (value: TPlugin['serializedValue']) => TPlugin['internalValue'] | TPlugin['emptyValue'];
} : {
    ...
    serialize?: (value: TPlugin['internalValue'] | TPlugin['emptyValue']) => TPlugin['serializedValue'];
} : {
...
export type PluginValue<TPlugin extends AnyPlugin> = Value<TPlugin['internalValue'], {
    serialized: TPlugin['serializedValue'];
    empty: TPlugin['emptyValue'];
}>;
export {};
